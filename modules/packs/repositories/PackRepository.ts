...
export class PackRepository implements IPackRepository {
  ...
  async create(
    ...
    const row = await this.db.kysely
      .insertInto('packs')
      // id/created_at/updated_at are generated by DB DEFAULT
      .values({
        client_id: clientId,
        name: data.name.trim(),
        price: String(data.price),
        old_price: data.old_price != null ? String(data.old_price) : null,
        primary_image_url: data.primary_image_url,
        image_urls: JSON.stringify(data.image_urls),
        features: JSON.stringify(data.features),
        sort_order: data.sort_order ?? 0,
      } as unknown as Parameters<ReturnType<typeof this.db.kysely.insertInto<'packs'>>['values']>[0])
      .returningAll()
